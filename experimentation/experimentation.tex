...
To get insight into Tribler's current query statistics, we have run two versions of Tribler for one hour using an empty state directly and a filled state directory containing 100,000 torrents and roughly 1200 channels discovered.
...
The breakdown of Tribler's database queries can be found in Table~\ref{table:query_breakdown_tribler_empty_state_dir} and Table~ , showing the breakdown of running Tribler idle for one hour with an empty and a filled state directory, respectively.
The breakdown of Dispersy's queries can be found in Table~\ref{table:query_breakdown_dispersy_empty_state_dir} and Table~. \todo{Run this experiment on the ubuntu 15.10 machine.}

From these breakdowns we observe that Tribler's database I/O has an enormous impact on Tribler's performance.
In the first experiment with an empty state directory, Dispersy generated X seconds worth of I/O using X queries in total. 
Tribler's database was queried X times, generating X seconds of I/O, Y\% more than Dispersy.
In total X out of 3600 seconds was spent on I/O, meaning Y\% of the time the main thread was blocked waiting for I/O.

The situation worsens when Tribler starts filling its state directory.
Using a filled state directory, Dispersy generated X seconds worth of I/O using X queries in total.
From this we observe that the queries are becoming more time consuming, which can be explained by the database having more data to search through. \todo{check if true.}
Tribler's database was queried X times, generating X seconds of I/O.
From these numbers we observe that Tribler's I/O time dramatically increased.
In this experiment out of 3600 seconds run-time, X was spent on I/O, meaning Y\% of the time the main thread was blocked.
...
From these results we conclude what was long a hunch, Tribler is I/O bound and in desperate need of non-blocking I/O.
By creating this tool, we have solved the Tribler's team latent need: insight into its I/O.
Using this tool, future developers can now focus on the queries which require optimization to adhere to the 80/20 rule: targeting the vital few, not the trivial many.
...
To look at the data generated by the benchmark in more detail, developers can look at the table generated by the regression test system, visible in Figure~\ref{fig:compare_table}.
