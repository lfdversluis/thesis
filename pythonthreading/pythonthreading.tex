...
To make sure threads running a Central Processing Unit (CPU) bound tasks do not hold the GIL indefinitely a simple check mechanism is built-in that \enquote{checks} every thread once per 100 ticks.
...
To illustrate the attempts to remove the GIL are still ongoing, Larry Hastings presented \enquote{The Gilectomy} at PyCon 2016.
